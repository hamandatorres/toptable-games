...
const getSecurityDashboard = async (req, res) => {
	try {
		...
		const suspiciousIPs = Object.entries(securityMetrics.ipStats)
			.filter(
				([ip, stats]) => stats.suspiciousActivity > 0 || stats.failedLogins > 5
			)
			.sort(
				([, a], [, b]) =>
					b.suspiciousActivity +
					b.failedLogins -
					(a.suspiciousActivity + a.failedLogins)
			)
			.slice(0, 10)
		...
		const activeUsers = Object.entries(securityMetrics.userStats)
			.filter(
				([, stats]) =>
					stats.lastLogin &&
					new Date() - new Date(stats.lastLogin) < 24 * 60 * 60 * 1000
			)
...
const calculateThreatLevel = () => {
	...
	const totalSuspicious =
		dailyStats.suspiciousActivity + dailyStats.rateLimitHits;
	const failureRate =
		dailyStats.totalLogins > 0
			? dailyStats.failedLogins / dailyStats.totalLogins
			: 0;
	...
	if (totalSuspicious > 10 || failureRate > 0.5) {
		...
		return {
			level: "MEDIUM",
			color: "#ffc107",
			description: "Some security events detected",
		};
	} else {
		return {
			level: "LOW",
