...
const SecurityDashboard: React.FC = () => {
	...
	const fetchDashboardData = async () => {
		try {
			setError(null);
			const response = await axios.get<SecurityDashboardData>(
	...
	const getEventTypeColor = (eventType: string) => {
		if (
			...
			eventType.includes("SQL_INJECTION")
		) {
			return "#dc3545"; // Red
		}
	...
	if (loading) {
		return (
			<div
				className="security-dashboard loading"
				role="status"
				aria-live="polite"
			>
	...
	return (
		<div className="security-dashboard" role="main">
					...
					<div className="stats-grid">
						<div className="stat">
							<span className="stat-value">
								{overview.dailyStats.totalLogins}
							</span>
							<span className="stat-label">Total Logins</span>
						</div>
						<div className="stat">
							<span className="stat-value">
								{overview.dailyStats.successfulLogins}
							</span>
							<span className="stat-label">Successful</span>
						</div>
						<div className="stat">
							<span className="stat-value">
								{overview.dailyStats.failedLogins}
							</span>
							<span className="stat-label">Failed</span>
						</div>
						<div className="stat">
							<span className="stat-value">
								{overview.dailyStats.suspiciousActivity}
							</span>
								...
								>
									...
									<div className="event-details">
										...
										{event.details.username && (
											<span>User: {event.details.username}</span>
										)}
										{event.details.extraData && (
											<span>
												Details: {JSON.stringify(event.details.extraData)}
											</span>
										)}
									</div>
								</div>
									...
									<div
										...
										style={{
											width: `${
												(reason.count /
													Math.max(
														...charts.topFailureReasons.map((r) => r.count)
													)) *
												100
											}%`,
										}}
			...
			<footer className="dashboard-footer">
				...
				<p>
					Active monitoring: {overview.totalIPs} IPs tracked,{" "}
